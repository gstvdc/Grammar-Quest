# Asset Attribution and Licenses

## Player Sprites

- **Source:** [CraftPix.net](https://craftpix.net/freebies/free-swordsman-1-3-level-pixel-top-down-sprite-character/)
- **Pack:** Free Swordsman 1-3 Level Pixel Top-Down Sprite Character
- **File:** `player/swordsman_walk.png` (from `PNG/Swordsman_lvl1/Without_shadow/Swordsman_lvl1_Walk_without_shadow.png`)
- **Format:** 384x256 px spritesheet (6 animation frames x 4 directions, 64x64 px per frame)
- **License:** CraftPix Standard Commercial & Personal License for 2D games

## Dungeon & Maze Tiles

- **Source:** [OpenGameArt.org](https://opengameart.org/content/sbs-top-down-dungeon-pack)
- **Pack:** SBS - Top Down Dungeon Pack
- **Files:**
  - `tiles/floor_metal.png` (64x64 px)
  - `tiles/wall_glass.png` (64x64 px)
  - `tiles/LICENSE-CC0.txt`
- **License:** Creative Commons CC0 1.0 Universal (Public Domain Dedication)
